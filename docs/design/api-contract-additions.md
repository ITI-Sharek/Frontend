...
> 3. **`PATCH /contributors/profiles/me`** `{ "bio"?: string|null, "availability"?: string|null, "experienceLevel"?: string|null, "interests"?: string[], "declaredSkills"?: string[] }` → full updated profile DTO with `completionPrompts` recomputed. The profile bio editor is built and working against a mock — edits currently revert on refetch because nothing persists (observed live). **New in this ask (2026-07-14)**: `experienceLevel` (one of `junior|mid|senior|expert`), `interests` (one of `web|mobile|ai|design|devops|docs`, repeatable), and `declaredSkills` (free-text tags, distinct from the AI-verified `skills` list) have no backend column at all yet — the registration "details step" already collects them but the data was previously discarded because `POST /auth/register` has no room for them and `ContributorProfile` (Prisma) only has `bio`/`availability`. The frontend now persists them client-side only (calls the mock `updateContributorProfileDetails` right after email verification via `ensureCurrentContributorProfile`); this needs 3 new nullable/array columns on `ContributorProfile` plus the PATCH payload above to actually survive a refresh.
...
> 8. **Plan/quota endpoint needed for the app-shell plan chip** (new, 2026-07-14): the sidebar shell (`src/routes/_appLayout.tsx`) is now role-aware — contributors see `الملف الشخصي / لوحة التحكم / ...`, owners see the smaller `مشاريعي / الإعدادات` shell per `docs/design/wireframes/09-owner-dashboard.md` §2.1 (the owner dashboard-variant route itself isn't built yet, so it's intentionally omitted rather than pointing at the contributor dashboard). Both roles' plan-chip quota labels (`"1 من 2 طلبات اليوم"` contributor / `"0 من 10 طلبات مساهمة هذا الشهر"` owner) are still fully hardcoded mock strings — no endpoint exists to report the current plan name or usage-this-period for either role. Needs something like `GET /billing/quota` → `{ planName, used, limit, periodLabel }`, role-aware.
