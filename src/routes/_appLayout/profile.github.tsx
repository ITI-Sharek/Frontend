...
export const Route = createFileRoute("/_appLayout/profile/github")({
  ...
  component: GithubWorkspacePage,
});

function GithubWorkspacePage() {
...
/**
 * Owner-facing GitHub App workspace. The authorization and candidate
 * completion protocol is deliberately the same as the contributor workflow,
 * while the result is limited to repositories that can be imported as
 * projects; it never exposes skill-analysis actions to an owner.
 */
function OwnerGithubRepositoriesPage() {
  const { t } = useTranslation();
  const search = Route.useSearch();
  const navigate = Route.useNavigate();
  const callbackPhase = getCallbackPhase(search);
  ...
  function startConnection(installationLinkId?: string) {
    ...
    startInstallationMutation.mutate(
      installationLinkId
        ...
        : { flowType: "install_and_authorize" },
      {
        onSuccess: (connection) => window.location.assign(connection.installationUrl),
  ...
  useEffect(() => {
    const attempt = attemptQuery.data;
    if (!attempt || autoCompletedAttemptRef.current === attempt.attemptId) return;
    const providerInstallationId = getAutoSelectableCandidateId(attempt.candidates);
    if (!providerInstallationId) return;
    autoCompletedAttemptRef.current = attempt.attemptId;
    completeAttempt(attempt.attemptId, providerInstallationId);
  }, [attemptQuery.data]);
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-4 px-4 py-6 md:px-6">
      ...
      {callbackPhase.kind === "resolving" && (
        <Card>
          {attemptQuery.isPending && (
            <p className="text-sm text-muted-foreground">{t("githubSkillPage.completing")}</p>
          )}
          {attemptQuery.isError && (
            <p role="alert" className="text-sm text-destructive">
              {getGitHubAppApiErrorMessage(attemptQuery.error)}
          ...
          {attemptQuery.data &&
            getAutoSelectableCandidateId(attemptQuery.data.candidates) === null && (
              <GitHubAppCandidatePicker
                candidates={attemptQuery.data.candidates}
                isSubmitting={completeInstallationMutation.isPending}
                errorMessage={actionError}
                onSelect={(providerInstallationId) =>
                  completeAttempt(attemptQuery.data.attemptId, providerInstallationId)
                }
                onRestart={() => {
                  clearCallbackSearch();
                  startConnection();
      ...
      <Card>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Button type="button" size="sm" variant="outline" disabled={isConnecting} onClick={() => startConnection()}>
            <Github className="size-4" />
            {t("project.ownerGithub.connect")}
          </Button>
        </div>
        {installationsQuery.isPending && (
          <p className="mt-4 text-xs text-muted-foreground">{t("githubSkillPage.loadingInstallations")}</p>
        )}
        {installationsQuery.isError && (
          <p role="alert" className="mt-4 text-xs text-destructive">
            {getGitHubAppApiErrorMessage(installationsQuery.error)}
        ...
        {installations.length > 0 && (
          <div className="mt-4">
            <GitHubAppInstallationList
              ...
              selectedInstallationLinkId={selectedInstallationLinkId}
              busyInstallationLinkId={
                disconnectMutation.isPending ? disconnectTarget?.installationLinkId ?? null : null
              }
      ...
      {disconnectTarget && (
        <GitHubAppDisconnectConfirm
          installation={disconnectTarget}
          isSubmitting={disconnectMutation.isPending}
          errorMessage={disconnectMutation.isError ? getGitHubAppApiErrorMessage(disconnectMutation.error) : null}
          onCancel={() => setDisconnectTarget(null)}
      ...
      {selectedInstallation && (
        <Card>
          <h2 className="text-sm font-bold text-foreground">{t("project.ownerGithub.availableRepositories")}</h2>
          <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
            {t("project.ownerGithub.availableRepositoriesDescription")}
          </p>
          {repositoriesQuery.isPending && (
            <p className="mt-4 text-xs text-muted-foreground">{t("githubApp.repositories.loading")}</p>
          )}
          {repositoriesQuery.isError && (
            <p role="alert" className="mt-4 text-xs text-destructive">
              {getGitHubAppApiErrorMessage(repositoriesQuery.error)}
          ...
          {repositoriesQuery.data?.items.map((repository) => (
            <p key={repository.repositoryId} dir="ltr" className="mt-3 font-mono text-sm text-foreground">
              {repository.fullName}
            </p>
          ))}
          <Button asChild className="mt-5">
            <a href={ROUTES.newProject}>{t("project.ownerGithub.import")}</a>
          </Button>
        </Card>
      )}
    </div>
