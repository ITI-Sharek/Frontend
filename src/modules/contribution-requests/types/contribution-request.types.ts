...
export interface ContributionRequestDto {
  ...
  /**
   * Present on a draft generated by accepting a Contribution Proposal. This is
   * the artefact the owner edits and publishes, so the credit has to travel
   * with it rather than living only on the proposal it came from.
   */
  attribution: {
    proposalId: string;
...
export interface ContributionRequestTechnologyFacetDto {
  technology: string;
  /** How many actionable Requests carry this tag, across all pages. */
  count: number;
}
